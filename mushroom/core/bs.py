...
def left_right_derivative_band(kx, band):
    """compute the left and right derivative of a band, i.e. partial E / partial k"""
    nk = len(kx)
    assert nk > 1
    right_derivs = np.zeros(nk)
    for i in range(nk - 1):
        if kx[i + 1] == kx[i]:
            right_derivs[i] = np.Inf
        else:
            right_derivs[i] = (band[i + 1] - band[i]) / (kx[i + 1] - kx[i])
    left_derivs = np.zeros(nk)
    left_derivs[1:] = right_derivs[0:-1]
    # impose that the fisrt/last point has equal left/right derivatives
    left_derivs[0] = right_derivs[0]
    right_derivs[-1] = left_derivs[-1]

    for i in range(1, nk):
        left_derivs[i] = right_derivs[i - 1]
        if left_derivs[i] == np.Inf:
            left_derivs[i] = left_derivs[i - 1]

    for i in range(nk - 1):
        if right_derivs[i] == np.Inf:
            right_derivs[i] = right_derivs[i + 1]
...
def resolve_band_crossing_2band(kx, band1, band2, deriv_thres=None):
    ...
    kx = np.array(kx)
    bands = np.array([band1, band2])

    # We scan every 3 points
    derivs_inband = np.zeros((nbands, 2))
    # cross-band derivatives, only 1 cross-band situation in 2-band case
    derivs_crossband = np.zeros((nbands, 2))
    # to generalize, one needs
    # derivs_crossband = np.zeros((np.math.factorial(nbands) - 1, nbands, 2))
    ...
    for i in range(1, nk - 1):
        ...
        # inband derivatives and crossband left derivatives
        derivs_inband[:, 0] = (bands_adjacent[:, 1] - bands_adjacent[:, 0]) / (kx[i] - kl)
        derivs_inband[:, 1] = (bands_adjacent[:, 2] - bands_adjacent[:, 3]) / (kx[i] - kr)
        derivs_crossband[:, 0] = (bands_adjacent[:, 1] - bands_adjacent[:, 0]) / (kx[i] - kl)
        # crossband right derivatives
        derivs_crossband[0, 1] = (bands_adjacent[0, 2] - bands_adjacent[1, 3]) / (kx[i] - kr)
        derivs_crossband[1, 1] = (bands_adjacent[1, 2] - bands_adjacent[0, 3]) / (kx[i] - kr)
        diff_derivs_inband = np.sum(np.abs(derivs_inband[:, 1] - derivs_inband[:, 0]))
        # diff_deriv_crossband = np.sum(np.abs(derivs_crossband[:, :, 1] - derivs_inband[:, :, 0]), axis=1)
        diff_deriv_crossband = np.sum(np.abs(derivs_crossband[:, 1] - derivs_inband[:, 0]))
        if diff_derivs_inband > diff_deriv_crossband + deriv_thres:
            _logger.debug("deriv %f > %f, possible crossing at %d, switch bands",
                          diff_derivs_inband, diff_deriv_crossband, i)
    ...
    _logger.debug("2-band resolve done")
